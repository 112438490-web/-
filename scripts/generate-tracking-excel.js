...
const assetsDirName = `\u7f3a\u9677\u8ddf\u8e2a_\u95ee\u9898\u7c7b\u578b\u7ba1\u7406_${reportDate}_assets`;
const assetsDir = path.join('reports', assetsDirName);
...
const copyScreenshot = (sourcePath, caseId, index) => {
  if (!sourcePath) return '';
  const ext = path.extname(sourcePath) || '.png';
  const safeExt = /\.(png|jpe?g)$/i.test(ext) ? ext : '.png';
  const fileName = `${caseId}_shot${index + 1}${safeExt}`;
  ...
  return `${assetsDirName}/${fileName}`.replace(/\\/g, '/');
};
...
const toHyperlinkFormula = (p, label) => {
  if (!p) return '';
  const safePath = toExcelPath(p);
  const safeLabel = String(label || '\u622a\u56fe').replace(/\"/g, '""');
  return `=HYPERLINK(\"${safePath}\",\"${safeLabel}\")`;
};
...
const rows = tests.map((t, idx) => {
  ...
  const screenshots = findScreenshots(attachments).slice(0, 2);
  return {
...
const toRowHtml = (cols) => `<tr>${cols.map((c) => `<td>${c}</td>`).join('')}</tr>`;
...
const rowsHtml = rows
  .map((r) => {
    ...
    const expected = bugId === '-' ? '' : expectedMap[r.title] || '\u89c1\u7528\u4f8b\u63cf\u8ff0';
    ...
    const shots =
      bugId === '-'
        ...
        : r.screenshots
            .map((p, idx) => {
              const rel = copyScreenshot(p, r.id, idx);
              return rel ? toHyperlinkFormula(rel, `\u622a\u56fe${idx + 1}`) : '';
            })
            .filter(Boolean)
            .join('<br/>');
    return toRowHtml([
      escapeHtml(r.id),
      escapeHtml(r.cnTitle),
      escapeHtml(result),
      escapeHtml(bugId),
      escapeHtml(defectStatus),
      escapeHtml(severity),
      escapeHtml(priority),
      escapeHtml(route),
      escapeHtml(r.steps),
...
<head>
  ...
  <meta charset="utf-8" />
  <title>\u7f3a\u9677\u8ddf\u8e2a_\u95ee\u9898\u7c7b\u578b\u7ba1\u7406_${reportDate}</title>
  <style>
...
const outPath = path.join('reports', `\u7f3a\u9677\u8ddf\u8e2a_\u95ee\u9898\u7c7b\u578b\u7ba1\u7406_${reportDate}.xls`);
const htmlWithBom = `\uFEFF${html}`;
fs.writeFileSync(outPath, htmlWithBom, 'utf8');
console.log(outPath);
